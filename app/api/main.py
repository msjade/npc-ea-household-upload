...
def parse_csv_file(file_bytes: bytes):
    ...
    if missing:
        return None, f"Missing column(s): {', '.join(sorted(missing))}. Use template: NAT_EA_SN, HOUSEHOLD_COUNT."
...
def upload(
    ...
    overwrite: str = Form("no"),  # Policy A default: no overwrite
    ...
    with engine.begin() as conn:
        for nat, hh in rows:
            # 1) Prevent same-client duplicates (same project/date/EA)
            ...
            # 2) Check if another client already updated this EA in master
            ...
            # 3) Always keep upload history record
            conn.execute(text("""
